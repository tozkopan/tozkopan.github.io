	...
	return UIComponent.extend("mvideo.cafe.Component", {
		...
		init: function() {
			...
			var fnIsWideScreen = function() {
				var bWide = /*oDeviceModel.getProperty("/system/tablet") ||*/
					oDeviceModel.getProperty("/system/desktop") || oDeviceModel.getProperty("/orientation/landscape");
			...
			if (oDeviceModel.getProperty("/mv_tablet")) {
				this.app.addStyleClass("MVideo_cafe_tablet");
			}
			this.app.attachBeforeMasterOpen(function(){
				console.log("attachBeforeMasterOpen");
				this.app.addStyleClass("MVideo_cafe_master_open");
			}.bind(this));
			this.app.attachAfterMasterClose(function(){
				console.log("attachAfterMasterClose");
				this.app.removeStyleClass("MVideo_cafe_master_open");
			}.bind(this));
			...
			oMainModel.attachMetadataLoaded(function() {
				...
				oMainModel.read("/TileGroupSet", {
					...
					success: function(data, response) {
						...
						oModel.setProperty("/TileGroupSet", data.results);
			...
			var fnReadLimits = function() {
				odataModel.read(sLimitURI, {
					...
					success: function(data, response) {
						...
						// data.OverdraftLimit = fDeducSum; //not from DMS
						if(data.Available < 0){
						...
						aLimits.forEach(function(item) {
							...
							if(item.id === "Overdraft"){
								item.color = item.price > 0 ? "red" : "none";
								item.price += " / " + data.OverdraftLimit;
							}
						});
						oModel.setProperty("/limits", aLimits);
						oModel.setProperty("/limitsRaw", data);
