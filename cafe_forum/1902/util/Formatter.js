...
mvideo.cafe.util.Formatter = {
	...
	tileEnabled : function(tile, obtained, groups, maxDmsRel, category) {
		if(!tile || !obtained) { return true; }
	//	var oCart = jQuery.grep(obtained, function(i){ return i.status == 20; })[0];
		var oCart = obtained;
	//	if(!oCart){ return true; }
		var aCart = [];
		oCart.forEach(function(cart) {
			aCart = aCart.concat(cart.content);
		});
		if(!aCart || !aCart.length){ return true; }//empty cart
		var maxBenInCart = tile.MaxNum ? +tile.MaxNum : 0;
		if (maxBenInCart) {
			var iBenInCart = $.grep(aCart,function(i){ return i.BenefitId == tile.BenefitId; }).length;
			return iBenInCart < maxBenInCart;
		}
		
		/*var iBenInCart = $.grep(aCart,function(i){ return i.BenefitId == tile.BenefitId; }).length; //TMP
		if (maxBenInCart && iBenInCart >= maxBenInCart) {
			return false;
		}*/
		var oGroup, oTile;
		groups.forEach(function(a) {
			oTile = jQuery.grep(a.tiles, function(i){ return i.id == tile.Tilegroup; })[0]; 
			if (oTile) {
				oGroup = oTile;
			}
		});
		//var oGroup = jQuery.grep(groups, function(i){ return i.id == tile.Tilegroup; })[0];
		if(oGroup){			
			for (var idx = 0; idx < aCart.length; idx++) {
				var cartItem = aCart[idx];
				if ( cartItem.BenefitId === tile.BenefitId 
						&& cartItem.Benefit2Id === tile.Benefit2Id ){
					// console.log( tile.Benefit2 + ": false self in cart");
					return false; // self in cart -> disable
				} else if( oGroup.groupDisable ) {
					for (var idx2 = 0; idx2 < oGroup.tiles.length; idx2++) {
						var oNeighbour = oGroup.tiles[idx2];
						if ( cartItem.BenefitId === oNeighbour.BenefitId 
								&& cartItem.Benefit2Id === oNeighbour.Benefit2Id ){
							// console.log( tile.Benefit2 + ": false neighbor in cart");
							return false; // groupDisable==true + neighbor in cart -> disable
						}
					}
				}
			}
		}
		// console.log( tile.Benefit2 + ": true default");
		return true; //default
	},
	...
	formatMoney : function(input, decimals, delimiter) {
	    ...
	    return parseFloat(input).toFixed(decimals).toString() + oBundle.getText("CMN_MONEY_ABBR");
	},
